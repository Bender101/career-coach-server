...
const apiHH = async (req, res) => {
  ...
  const requestAmount = await axios(
    `https://api.hh.ru/vacancies/?text=${title}&search_field=name&period=${period}&area=${areaCode}`,
    {
      method: "get",
      headers: {
  ...
  for (let page = 0; page < pages; page++) {
    const requestString = salary
      ? `https://api.hh.ru/vacancies/?text=${title}&search_field=name&salary=${salary}&period=${period}&per_page=${perPage}&page=${page}&area=${areaCode}&order_by=publication_time`
    ...
    for (let i = 0; i < vacancies.length; i++) {
      let bestVacsObj = {};
      const oneVacancy = await axios(
        `https://api.hh.ru/vacancies/${vacancies[i].id}`,
        {
          method: "get",
          headers: {
            "User-Agent": "CareerCoach (cska2004@gmail.com)",
      ...
      if (!vacancies[i].salary) {
        ...
        if (vacancies[i].salary?.from) {
          salaryStr =
            vacancies[i].salary?.to !== undefined &&
            vacancies[i].salary?.to !== null
              ? "от " +
  ...
  if (Object.keys(skillsObj).length !== 0) {
    ...
    const erase = await BestVacancy.destroy({
